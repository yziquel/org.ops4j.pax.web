...
public class WebAppParser {
    ...
    public void parse(final Bundle bundle, WebApp webApp) throws Exception {
        ...
        if (webXmlURL != null) {
            ...
            try {
                ...
                boolean metaDataComplete = parseBoolean(getAttribute(rootElement, "metadata-complete", "false"));
    ...
    private void tldScan(final Bundle bundle, final WebApp webApp) throws Exception {
        ...
        for (Bundle bundleInClassSpace : bundlesInClassSpace) {
            ...
            e = bundleInClassSpace.findEntries("/META-INF", "*.taglib.xml", false);
            if (e != null) {
                while (e.hasMoreElements()) {
                    URL u = (URL) e.nextElement();
            ...
            e = bundleInClassSpace.findEntries("/META-INF", "faces-config.xml", false);
            if (e != null) {
                while (e.hasMoreElements()) {
                    URL u = (URL) e.nextElement();
    ...
    private void servletContainerInitializerScan(Bundle bundle, WebApp webApp, Integer majorVersion) throws Exception {
        ...
        if (serviceLoader != null) {
            ...
            for (Entry<ServletContainerInitializer, Class<ServletContainerInitializer>> service : serviceLoader.entrySet()) {
                ...
                webAppServletContainerInitializer.setServletContainerInitializer(service.getKey());
                ...
                for (Element scElement : securityConstraint) {
                    ...
                    if (userDataConstraintsElement != null) {
                        String guarantee = getTextContent(
                                getChild(userDataConstraintsElement, "transport-guarantee")).trim().toUpperCase();
                    ...
                    if (webResourceElements != null && webResourceElements.length > 0) {
                        for (Element webResourceElement : webResourceElements) {
                            ...
                            String constraintName = getTextContent(getChild(webResourceElement, "web-resource-name"));
    ...
    private static void parseServlets(final Element rootElement, final WebApp webApp) {
        ...
        if (elements != null && elements.length > 0) {
            for (Element element : elements) {
                ...
                String servletClass = getTextContent(getChild(element, "servlet-class"));
